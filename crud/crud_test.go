...
func TestCrud_Get(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/user/get", bytes.NewBuffer(body))
	r.ServeHTTP(res, req)
	assert.Equal(t,
		res.Body.String(),
...
func TestCrud_OriginLists(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/user/originLists", bytes.NewBuffer(body))
	r.ServeHTTP(res, req)
	assert.Equal(t,
		res.Body.String(),
...
func TestCrud_Lists(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/user/lists", bytes.NewBuffer(body))
	r.ServeHTTP(res, req)
	assert.Equal(t,
		res.Body.String(),
...
func TestCrud_Edit(t *testing.T) {
	...
	req, _ := http.NewRequest("POST", "/user/edit", bytes.NewBuffer(body))
	r.ServeHTTP(res, req)
	assert.Equal(t,
		res.Body.String(),
		`{"error":0,"msg":"ok"}`,

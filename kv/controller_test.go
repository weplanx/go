...
func TestSetBadValidate(t *testing.T) {
	...
	w := ut.PerformRequest(r, "PATCH", "/values",
...
func TestGet(t *testing.T) {
	err := service.Load()
	assert.NoError(t, err)
	w := ut.PerformRequest(r, "GET", "/values",
		&ut.Body{},
		ut.Header{Key: "content-type", Value: "application/json"},
	)
	resp := w.Result()
...
func TestRemoveBadValidate(t *testing.T) {
	w := ut.PerformRequest(r, "DELETE", fmt.Sprintf(`/values/%s`, "123456"),
		&ut.Body{},
		ut.Header{Key: "content-type", Value: "application/json"},
	)
	resp := w.Result()
...
func TestRemove(t *testing.T) {
	err := service.Load()
	assert.NoError(t, err)
	w := ut.PerformRequest(r, "DELETE", fmt.Sprintf(`/values/%s`, "login_ttl"),
		&ut.Body{},
		ut.Header{Key: "content-type", Value: "application/json"},
	)
	resp := w.Result()
...
func TestRemoveBadService(t *testing.T) {
	err := keyvalue.Delete("values")
	assert.NoError(t, err)
	w := ut.PerformRequest(r, "DELETE", fmt.Sprintf(`/values/%s`, "login_failures"),
		&ut.Body{},
		ut.Header{Key: "content-type", Value: "application/json"},
	)
	resp := w.Result()
